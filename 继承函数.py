...
class FullTimeEmployee(Employee):
    def __init__(self, name, id, monthly_salary):    #继承只是代表:子类可以使用父类的参数，但不会自动帮你填参数
        super().__init__(name, id)                   #调用父类的Employee的初始化函数，帮我把name和id这部分处理掉，就不用写 self.name = name 和 self.id = id
        self.monthly_salary = monthly_salary         #再加一个参数

    def calculate_monthly_salary(self):
        return self.monthly_salary                   #返回return后面的数值，（提交那个数值）
